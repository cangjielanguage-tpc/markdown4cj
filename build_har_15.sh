#!/usr/bin/env bash
# =============================================================================
# build_har_15.sh —— Linux / macOS 版本（对应 Windows 的 build_har_15.ps1）
#
# 流程（按顺序）：
#   1. 依据 .gitmodules 下载 4 个子模块（prism4cj / codeformat4cj / markdown4cj / formula4cj）
#   2. 将各子模块下所有 build-profile.json5 的 compatibleSdkVersion 统一改为 5.0.3(15)
#   3. 依次打包子模块 HAR 并复制到根项目 markdown_arkui/har 目录：
...
#   4. 在根项目编译 markdown_arkui 模块（assembleHar）
#
# 用法：
#   bash build_har_15.sh                                # 完整流程
#   bash build_har_15.sh --only prism4cj,formula4cj     # 第 3 步只打包指定模块
#   bash build_har_15.sh --clean                        # 打包前删除各模块的旧 build 目录
#   bash build_har_15.sh --skip-download                # 子模块已就绪时跳过清理与下载
#   bash build_har_15.sh --build-mode debug             # 构建模式（默认 release）
#   bash build_har_15.sh --deveco-tools /opt/deveco/tools \
...
# ---------------- 步骤2：统一 compatibleSdkVersion 为 5.0.3(15) ----------------
# 覆盖根项目（根目录 build-profile.json5 的 app.products）与全部 4 个子模块
TARGET_SDK_COMPAT='5.0.3(15)'
while IFS= read -r pf; do
    [ -n "$pf" ] || continue
    if grep -Eq '"compatibleSdkVersion"[[:space:]]*:[[:space:]]*"[^"]*"' "$pf"; then
        before="$(cat "$pf")"
        perl -i -pe 's/"compatibleSdkVersion"\s*:\s*"[^"]*"/"compatibleSdkVersion": "5.0.3(15)"/g' "$pf"
        if [ "$before" != "$(cat "$pf")" ]; then
            echo "  已将 compatibleSdkVersion 改为 $TARGET_SDK_COMPAT: ${pf#$PROJECT_ROOT/}"
        fi
    fi
done < <(find "$PROJECT_ROOT" -type f -name 'build-profile.json5' \
            -not -path '*/oh_modules/*' -not -path '*/build/*' -not -path '*/node_modules/*' \
            -not -path '*/.git/*' -not -path '*/.hvigor_home/*' 2>/dev/null)
