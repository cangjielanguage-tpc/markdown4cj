<#
.SYNOPSIS
  下载子模块源码 -> 修正 compatibleSdkVersion -> 批量打包子模块 HAR -> 编译 markdown_arkui 模块。
...
.EXAMPLE
  ...
  # 第 3 步只打包指定模块（第 1/2 步仍作用于全部 4 个子模块）
...
.EXAMPLE
  powershell -ExecutionPolicy Bypass -File .\build_har.ps1 -DevEcoTools 'C:\DevEco\tools' -CangjieSdkRoot 'C:\compat-sdk\compatibility'
...
foreach ($pf in $profiles) {
    ...
    if ($text -match '"compatibleSdkVersion"\s*:\s*"[^"]*"') {
        ...
        if ($newText -ne $text) {
            ...
            Write-Host ("  已将 compatibleSdkVersion 改为 {0}: {1}" -f $TargetSdkCompat, $pf.FullName.Substring($ProjectRoot.Length + 1))
        }
    }
}
